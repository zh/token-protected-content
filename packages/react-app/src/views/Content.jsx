...
function Content({ userSigner, web3Modal }) {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`${AUTH_SERVER}/signIn`, { signature, message, address });
      ...
      {token && (
        <div style={{ marginTop: 60 }}>
          <video width="400" height="400" controls>
            <source src={`${AUTH_SERVER}/video?token=${token}`} type="video/mp4" />
